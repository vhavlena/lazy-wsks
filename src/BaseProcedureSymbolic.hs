...
minusSymbolSym t1@(TStates aut1 var1 st1) t2@(TStates aut2 var2 st2) sym =
  unifySymTerm lst where
    lst = [(minusSymbol t1 t2 s, convSymToFle s) | s <- unsyms]
...
minusSymbolSym (TProj v1 t1) (TProj v2 t2) sym
  ...
  | v1 == v2 = mergeEquvalent $ map g l
    where
      ...
      --g (t, f) = (TProj v1 t, f)
      g (t, f) = (TProj v1 t, (rplSatVar f var Yes) :||: (rplSatVar f var No))
minusSymbolSym (TSet tset1) (TSet tset2) sym = filter (isSat) coLst where
  cr = List.nub $ Aux.crossProdLst $ [minusSymbolSym t1 t2 sym | t1 <- Set.toList tset1, t2 <- Set.toList tset2]
...
mergeEquvalent s = [(joinTerm t1 t2, f1) | (t1,f1) <- s, (t2,f2) <- s, isSat ((Not f1) :||: f2) (f1 :||: (Not f2))]
...
unifySymTerm = Map.toList . Map.fromListWith (:||:)
...
satTermProductWith l1 l2 cons = [(cons t1 t2, f1 :&&: f2) | (t1, f1) <- l1, (t2, f2) <- l2, isSat f1 f2]
